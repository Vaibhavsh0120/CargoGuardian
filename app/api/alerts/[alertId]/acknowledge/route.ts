...
import { acknowledgeAlertById } from "@/services/alerts/write";
...
export async function POST(_request: Request, { params }: { params: Promise<{ alertId: string }> }) {
  ...
  if (user.role !== "admin" && user.role !== "master") {
    return failure("Only admins and masters can acknowledge alerts.", 403);
  }

  const { alertId } = await params;
  const alert = await acknowledgeAlertById(alertId, {
    actorId: user.uid,
